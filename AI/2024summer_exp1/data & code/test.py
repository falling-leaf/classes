...
if __name__ == '__main__':
    ...
    for i in range(100):
        batch_images = test_images[cnt:(cnt + 100)]
        batch_labels = test_labels[cnt:(cnt + 100)]
        ...
        result_lst.append(accuracy)
        end_time = time.time()
        print("processing time: {:.3f}s".format(end_time - start_time))
        cnt += 100
    with open('result_lst.txt', 'a') as file:
        file.write(str(result_lst) + "\n")
    print("--------------------------")
    print("all data in test finished.")
    print("the average accuracy is:{:.2f}%".format(sum_accuracy))
    x = [i * 100 for i in range(len(result_lst))]
    y = [i for i in result_lst]
    plt.figure()
    plt.plot(x, y)  
    # 添加标题和坐标轴标签
    plt.ylim(0, 1)  
    plt.title('average accuracy of the test is:{:}'.format(sum_accuracy))  
    plt.xlabel('x axis')  
    plt.ylabel('y axis')  
