...
export default {
    ...
    methods: {
        ConfirmNewCard() {
            // 发出POST请求
            console.log(this.toModifyInfo.type)
            axios.post("/card",
                { // 请求体
        ...
        ConfirmRemoveCard() {
            axios.delete('/card', { params: { toRemove: this.toRemove } })
                .then(response =>{
                    if (response.data === "Card deleted successfully"){
                        ElMessage.success("借书证删除成功")
                        this.removeCardVisible = false
                        this.QueryCards()
                    } else {
                        ElMessage.error("借书证删除失败，该借书证有书未还~")
                        this.removeCardVisible = false
                        this.QueryCards()
                    }   
                })
